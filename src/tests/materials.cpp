...
Real compute_determinant(const Material& m, const PathVertex& vertex, const Vector3& dir_in, const Vector2& rnd_param,
                         Real w) {
    Real eps                               = Real(1e-6);
    std::optional<BSDFSampleRecord> sample = sample_bsdf(m, dir_in, vertex, TexturePool(), rnd_param, w);
    std::optional<BSDFSampleRecord> sample_u =
        sample_bsdf(m, dir_in, vertex, TexturePool(), rnd_param + Vector2{ eps, Real(0) }, w);
    std::optional<BSDFSampleRecord> sample_v =
        sample_bsdf(m, dir_in, vertex, TexturePool(), rnd_param + Vector2{ Real(0), eps }, w);
...
int main(int argc, char* argv[]) {
    ...
    {
        Material m = RoughPlastic{ ConstantTexture<Spectrum>{ Vector3{ Real(0.5), Real(0.5), Real(0.5) } },
                                   ConstantTexture<Spectrum>{ Vector3{ Real(0.5), Real(0.5), Real(0.5) } },
                                   ConstantTexture<Real>{ Real(0.3) }, Real(1.5) };
    ...
    {
        Material m = RoughDielectric{ ConstantTexture<Spectrum>{ Vector3{ Real(0.5), Real(0.5), Real(0.5) } },
                                      ConstantTexture<Spectrum>{ Vector3{ Real(0.5), Real(0.5), Real(0.5) } },
                                      ConstantTexture<Real>{ Real(0.3) }, Real(1.5) };
