...
enum class TextureType { BITMAP, CHECKERBOARD };
...
enum class FovAxis { X, Y, DIAGONAL, SMALLER, LARGER };
...
auto parse_default_map(const std::string& value, const std::map<std::string, std::string>& default_map) {
    if (value.length() > 0) {
        if (value[0] == '$') {
...
bool parse_boolean(const std::string& value, const std::map<std::string, std::string>& default_map) {
    if (auto it = parse_default_map(value, default_map); it != default_map.end()) { return parse_boolean(it->second); }
    return parse_boolean(value);
}

int parse_integer(const std::string& value, const std::map<std::string, std::string>& default_map) {
    if (auto it = parse_default_map(value, default_map); it != default_map.end()) { return std::stoi(it->second); }
    return std::stoi(value);
}

Real parse_float(const std::string& value, const std::map<std::string, std::string>& default_map) {
    if (auto it = parse_default_map(value, default_map); it != default_map.end()) { return std::stof(it->second); }
    return std::stof(value);
}

std::string parse_string(const std::string& value, const std::map<std::string, std::string>& default_map) {
    if (auto it = parse_default_map(value, default_map); it != default_map.end()) { return it->second; }
    return value;
}
...
Vector3 parse_vector3(const std::string& value, const std::map<std::string, std::string>& default_map) {
    if (auto it = parse_default_map(value, default_map); it != default_map.end()) { return parse_vector3(it->second); }
...
Vector3 parse_srgb(const std::string& value) {
    ...
    if (value.size() == 7 && value[0] == '#') {
        ...
        if (*end_ptr != '\0') { Error(std::string("Invalid SRGB value: ") + value); }
...
Vector3 parse_srgb(const std::string& value, const std::map<std::string, std::string>& default_map) {
    if (auto it = parse_default_map(value, default_map); it != default_map.end()) { return parse_srgb(it->second); }
...
std::vector<std::pair<Real, Real>> parse_spectrum(const std::string& value) {
    ...
    if (list.size() == 1 && list[0].find(":") == std::string::npos) {
        ...
        for (auto val_str : list) {
            ...
            if (pair.size() < 2) { Error("parse_spectrum failed"); }
...
std::vector<std::pair<Real, Real>> parse_spectrum(const std::string& value,
    ...
    if (auto it = parse_default_map(value, default_map); it != default_map.end()) { return parse_spectrum(it->second); }
    return parse_spectrum(value);
}

Matrix4x4 parse_matrix4x4(const std::string& value) {
    std::vector<std::string> list = split_string(value, std::regex("(,| )+"));
    if (list.size() != 16) { Error("parse_matrix4x4 failed"); }

    Matrix4x4 m;
    int k = 0;
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) { m(i, j) = std::stof(list[k++]); }
    }
    return m;
}

Matrix4x4 parse_matrix4x4(const std::string& value, const std::map<std::string, std::string>& default_map) {
...
Matrix4x4 parse_transform(pugi::xml_node node, const std::map<std::string, std::string>& default_map) {
    ...
    for (auto child : node.children()) {
        ...
        if (name == "scale") {
            ...
            Real y = 1.0;
            Real z = 1.0;
            if (!child.attribute("x").empty()) { x = parse_float(child.attribute("x").value(), default_map); }
            if (!child.attribute("y").empty()) { y = parse_float(child.attribute("y").value(), default_map); }
            if (!child.attribute("z").empty()) { z = parse_float(child.attribute("z").value(), default_map); }
            if (!child.attribute("value").empty()) {
                Vector3 v = parse_vector3(child.attribute("value").value(), default_map);
                x         = v.x;
                y         = v.y;
                z         = v.z;
            }
            ...
            Vector3 pos    = parse_vector3(child.attribute("origin").value(), default_map);
            Vector3 target = parse_vector3(child.attribute("target").value(), default_map);
            Vector3 up     = parse_vector3(child.attribute("up").value(), default_map);
            tform          = look_at(pos, target, up) * tform;
        } else if (name == "matrix") {
            Matrix4x4 trans = parse_matrix4x4(std::string(child.attribute("value").value()), default_map);
            tform           = trans * tform;
        }
    }
    return tform;
...
Spectrum parse_color(pugi::xml_node node, const std::map<std::string, std::string>& default_map) {
    std::string type = node.name();
    if (type == "spectrum") {
        std::vector<std::pair<Real, Real>> spec = parse_spectrum(node.attribute("value").value(), default_map);
        if (spec.size() > 1) {
...
ParsedTexture parse_texture(pugi::xml_node node, const std::map<std::string, std::string>& default_map) {
    ...
    if (type == "bitmap") {
        ...
        for (auto child : node.children()) {
            ...
            if (name == "filename") {
                ...
                uscale = vscale = parse_float(child.attribute("value").value(), default_map);
            } else if (name == "uscale") {
                uscale = parse_float(child.attribute("value").value(), default_map);
            } else if (name == "vscale") {
                vscale = parse_float(child.attribute("value").value(), default_map);
            } else if (name == "uoffset") {
                uoffset = parse_float(child.attribute("value").value(), default_map);
            } else if (name == "voffset") {
                voffset = parse_float(child.attribute("value").value(), default_map);
            }
        }
...
Texture<Spectrum> parse_spectrum_texture(pugi::xml_node node,
    ...
    if (type == "spectrum") {
        ...
        return make_constant_spectrum_texture(fromRGB(parse_vector3(node.attribute("value").value(), default_map)));
    } else if (type == "srgb") {
        Vector3 srgb = parse_srgb(node.attribute("value").value(), default_map);
        return make_constant_spectrum_texture(fromRGB(sRGB_to_RGB(srgb)));
    } else if (type == "ref") {
        // referencing a texture
        std::string ref_id = node.attribute("id").value();
        auto t_it          = texture_map.find(ref_id);
        if (t_it == texture_map.end()) { Error(std::string("Texture not found. ID = ") + ref_id); }
        const ParsedTexture t = t_it->second;
        if (t.type == TextureType::BITMAP) {
            return make_image_spectrum_texture(ref_id, t.filename, texture_pool, t.uscale, t.vscale, t.uoffset,
                                               t.voffset);
        } else if (t.type == TextureType::CHECKERBOARD) {
            return make_checkerboard_spectrum_texture(t.color0, t.color1, t.uscale, t.vscale, t.uoffset, t.voffset);
        } else {
            return make_constant_spectrum_texture(fromRGB(Vector3{ 0, 0, 0 }));
        }
        ...
        while (texture_id_exists(texture_pool, tmp_ref_name + std::to_string(ref_id_counter))) { ref_id_counter++; }
        tmp_ref_name = tmp_ref_name + std::to_string(ref_id_counter);
        if (t.type == TextureType::BITMAP) {
            return make_image_spectrum_texture(tmp_ref_name, t.filename, texture_pool, t.uscale, t.vscale, t.uoffset,
                                               t.voffset);
        } else if (t.type == TextureType::CHECKERBOARD) {
            return make_checkerboard_spectrum_texture(t.color0, t.color1, t.uscale, t.vscale, t.uoffset, t.voffset);
        } else {
...
Texture<Real> parse_float_texture(pugi::xml_node node,
                                  const std::map<std::string /* name id */, ParsedTexture>& texture_map,
                                  TexturePool& texture_pool, const std::map<std::string, std::string>& default_map) {
    std::string type = node.name();
    if (type == "ref") {
        // referencing a texture
        std::string ref_id = node.attribute("id").value();
        ...
        const ParsedTexture t = t_it->second;
        if (t.type == TextureType::BITMAP) {
            return make_image_float_texture(ref_id, t.filename, texture_pool, t.uscale, t.vscale, t.uoffset, t.voffset);
        } else if (t.type == TextureType::CHECKERBOARD) {
            return make_checkerboard_float_texture(avg(t.color0), avg(t.color1), t.uscale, t.vscale, t.uoffset,
                                                   t.voffset);
        } else {
            return make_constant_float_texture(0);
        }
    } else if (type == "float") {
        return make_constant_float_texture(parse_float(node.attribute("value").value(), default_map));
    } else if (type == "texture") {
        ParsedTexture t = parse_texture(node, default_map);
        // increment ref_id_counter until we can't find the name in texture_pool
        static int ref_id_counter = 0; // should switch to atomic<int> if we parse scenes using multiple threads
        ...
        tmp_ref_name = tmp_ref_name + std::to_string(ref_id_counter);
        if (t.type == TextureType::BITMAP) {
            return make_image_float_texture(tmp_ref_name, t.filename, texture_pool, t.uscale, t.vscale, t.uoffset,
                                            t.voffset);
        } else if (t.type == TextureType::CHECKERBOARD) {
            return make_checkerboard_float_texture(avg(t.color0), avg(t.color1), t.uscale, t.vscale, t.uoffset,
                                                   t.voffset);
...
Spectrum parse_intensity(pugi::xml_node node, const std::map<std::string, std::string>& default_map) {
...
void parse_default_map(pugi::xml_node node, std::map<std::string, std::string>& default_map) {
...
RenderOptions parse_integrator(pugi::xml_node node, const std::map<std::string, std::string>& default_map) {
    ...
    if (type == "path") {
        ...
        for (auto child : node.children()) {
            std::string name = child.attribute("name").value();
            if (name == "maxDepth") {
                options.max_depth = parse_integer(child.attribute("value").value(), default_map);
            } else if (name == "rrDepth") {
                options.rr_depth = parse_integer(child.attribute("value").value(), default_map);
            }
        }
        ...
        for (auto child : node.children()) {
            ...
            if (name == "maxDepth" || name == "max_depth") {
                ...
                options.vol_path_version = parse_integer(child.attribute("value").value(), default_map);
            } else if (name == "maxNullCollisions" || name == "max_null_collisions") {
                options.max_null_collisions = parse_integer(child.attribute("value").value(), default_map);
            }
        }
    } else if (type == "direct") {
        options.integrator = Integrator::Path;
            ...
            if (filter_type == "box") {
                ...
                for (auto grand_child : child.children()) {
                    if (std::string(grand_child.attribute("name").value()) == "width") {
                        filter_width = parse_float(grand_child.attribute("value").value(), default_map);
                    }
                }
                ...
                for (auto grand_child : child.children()) {
                    if (std::string(grand_child.attribute("name").value()) == "stddev") {
                        filter_stddev = parse_float(grand_child.attribute("value").value(), default_map);
                    }
                }
...
VolumeSpectrum parse_volume_spectrum(pugi::xml_node node, const std::map<std::string, std::string>& default_map) {
    std::string type = node.attribute("type").value();
    if (type == "constvolume") {
        Spectrum value = make_zero_spectrum();
        for (auto child : node.children()) {
            std::string name = child.attribute("name").value();
            if (name == "value") { value = parse_color(child, default_map); }
        }
        ...
        for (auto child : node.children()) {
            std::string name = child.attribute("name").value();
            if (name == "filename") { filename = parse_string(child.attribute("value").value(), default_map); }
        }
        if (filename.empty()) { Error("Empty filename for a gridvolume."); }
...
PhaseFunction parse_phase_function(pugi::xml_node node, const std::map<std::string, std::string>& default_map) {
    std::string type = node.attribute("type").value();
    if (type == "isotropic") {
        ...
        for (auto child : node.children()) {
            std::string name = child.attribute("name").value();
            if (name == "g") { g = parse_float(child.attribute("value").value(), default_map); }
        }
...
parse_sensor(pugi::xml_node node, std::vector<Medium>& media,
    ...
    for (auto child : node.children()) {
        if (std::string(child.name()) == "film") {
            ...
            if (id.empty()) { Error("Medium reference not specified."); }
            ...
            if (!medium_name.empty()) { medium_map[medium_name] = media.size(); }
...
Texture<Real> alpha_to_roughness(pugi::xml_node node,
    ...
    if (type == "ref") {
        ...
        if (t.type == TextureType::BITMAP) {
            ...
            for (int i = 0; i < alpha.width * alpha.height; i++) { roughness_img.data[i] = sqrt(alpha.data[i]); }
...
parse_bsdf(pugi::xml_node node, const std::map<std::string /* name id */, ParsedTexture>& texture_map,
           TexturePool& texture_pool, const std::map<std::string, std::string>& default_map,
    ...
    if (type == "twosided") {
        ...
        for (auto child : node.children()) {
            std::string name = child.attribute("name").value();
            if (name == "reflectance") {
                reflectance = parse_spectrum_texture(child, texture_map, texture_pool, default_map);
            }
        }
        ...
        for (auto child : node.children()) {
            ...
            if (name == "diffuseReflectance" || name == "diffuse_reflectance") {
                diffuse_reflectance = parse_spectrum_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "specularReflectance" || name == "specular_reflectance") {
                specular_reflectance = parse_spectrum_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "alpha") {
        ...
        return std::make_tuple(id,
                               RoughPlastic{ diffuse_reflectance, specular_reflectance, roughness, intIOR / extIOR });
        ...
        for (auto child : node.children()) {
            ...
            if (name == "specularReflectance" || name == "specular_reflectance") {
                ...
                specular_transmittance = parse_spectrum_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "alpha") {
                roughness = alpha_to_roughness(child, texture_map, texture_pool, default_map);
            } else if (name == "roughness") {
                roughness = parse_float_texture(child, texture_map, texture_pool, default_map);
        ...
        for (auto child : node.children()) {
            ...
            if (name == "baseColor" || name == "base_color") {
                base_color = parse_spectrum_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "roughness") {
                roughness = parse_float_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "subsurface") {
                subsurface = parse_float_texture(child, texture_map, texture_pool, default_map);
            }
        }
        ...
        for (auto child : node.children()) {
            ...
            if (name == "baseColor" || name == "base_color") {
                ...
                anisotropic = parse_float_texture(child, texture_map, texture_pool, default_map);
            }
        }
        ...
        for (auto child : node.children()) {
            ...
            if (name == "clearcoatGloss") {
                clearcoat_gloss = parse_float_texture(child, texture_map, texture_pool, default_map);
            }
        }
        ...
        for (auto child : node.children()) {
            ...
            if (name == "baseColor" || name == "base_color") {
                ...
                sheen_tint = parse_float_texture(child, texture_map, texture_pool, default_map);
            }
        }
        ...
        for (auto child : node.children()) {
            ...
            if (name == "baseColor" || name == "base_color") {
                base_color = parse_spectrum_texture(child, texture_map, texture_pool, default_map);
                       ...
                       name == "spec_trans") {
                specular_transmission = parse_float_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "metallic") {
                metallic = parse_float_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "subsurface") {
                subsurface = parse_float_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "specular") {
                specular = parse_float_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "roughness") {
                roughness = parse_float_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "specularTint" || name == "specular_tint" || name == "specTint" || name == "spec_tint") {
                specular_tint = parse_float_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "anisotropic") {
                anisotropic = parse_float_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "sheen") {
                sheen = parse_float_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "sheenTint" || name == "sheen_tint") {
                sheen_tint = parse_float_texture(child, texture_map, texture_pool, default_map);
            } else if (name == "clearcoat") {
                clearcoat = parse_float_texture(child, texture_map, texture_pool, default_map);
        ...
        return std::make_tuple(id, DisneyBSDF{ base_color, specular_transmission, metallic, subsurface, specular,
                                               roughness, specular_tint, anisotropic, sheen, sheen_tint, clearcoat,
                                               clearcoat_gloss, eta });
    } else if (type == "null") {
        // TODO: implement actual null BSDF (the ray will need to pass through the shape)
        return std::make_tuple(id, Lambertian{ make_constant_spectrum_texture(fromRGB(Vector3{ 0.0, 0.0, 0.0 })) });
    } else {
...
Shape parse_shape(pugi::xml_node node, std::vector<Material>& materials,
                  std::map<std::string /* name id */, int /* index id */>& material_map,
                  const std::map<std::string /* name id */, ParsedTexture>& texture_map, TexturePool& texture_pool,
                  std::vector<Medium>& media, std::map<std::string /* name id */, int /* index id */>& medium_map,
                  std::vector<Light>& lights, const std::vector<Shape>& shapes,
                  const std::map<std::string, std::string>& default_map) {
    int material_id        = -1;
    ...
    for (auto child : node.children()) {
        ...
        if (name == "ref") {
            std::string name_value = child.attribute("name").value();
            pugi::xml_attribute id = child.attribute("id");
            if (id.empty()) { Error("Material/medium reference id not specified."); }
            ...
            std::tie(material_name, m) = parse_bsdf(child, texture_map, texture_pool, default_map);
            if (!material_name.empty()) { material_map[material_name] = materials.size(); }
    ...
    if (type == "obj") {
        ...
        for (auto child : node.children()) {
            ...
            if (name == "filename") {
                ...
                if (std::string(child.name()) == "transform") { to_world = parse_transform(child, default_map); }
            } else if (name == "faceNormals" || name == "face_normals") {
                face_normals = parse_boolean(child.attribute("value").value(), default_map);
            }
        }
        ...
        if (face_normals) {
            ...
            if (mesh.normals.size() == 0) { mesh.normals = compute_normal(mesh.positions, mesh.indices); }
        }
        ...
        for (auto child : node.children()) {
            std::string name = child.attribute("name").value();
            if (name == "center") {
                center = Vector3{ parse_float(child.attribute("x").value(), default_map),
        ...
        TriangleMesh mesh;
        mesh.positions = { Vector3{ -1, -1, 0 }, Vector3{ 1, -1, 0 }, Vector3{ 1, 1, 0 }, Vector3{ -1, 1, 0 } };
        mesh.indices   = { Vector3i{ 0, 1, 2 }, Vector3i{ 0, 2, 3 } };
        mesh.uvs       = { Vector2{ 0, 0 }, Vector2{ 1, 0 }, Vector2{ 1, 1 }, Vector2{ 0, 1 } };
        ...
        if (flip_normals) {
            for (auto& n : mesh.normals) { n = -n; }
        }
        for (auto& p : mesh.positions) { p = xform_point(to_world, p); }
        for (auto& n : mesh.normals) { n = xform_normal(inverse(to_world), n); }
        shape = mesh;
    } else {
    ...
    for (auto child : node.children()) {
        ...
        if (name == "emitter") {
            ...
            for (auto grand_child : child.children()) {
                std::string name = grand_child.attribute("name").value();
                if (name == "radiance") { radiance = parse_intensity(grand_child, default_map); }
            }
...
std::unique_ptr<Scene> parse_scene(pugi::xml_node node, const RTCDevice& embree_device) {
    RenderOptions options;
    Camera camera(Matrix4x4::identity(), c_default_fov, c_default_res, c_default_res, c_default_filter,
                  -1 /*medium_id*/);
    std::string filename = c_default_filename;
    ...
    for (auto child : node.children()) {
        ...
        if (name == "default") {
            ...
            ParsedSampler sampler;
            std::tie(camera, filename, sampler) = parse_sensor(child, media, medium_map, default_map);
            ...
            Shape s = parse_shape(child, materials, material_map, texture_map, texture_pool, media, medium_map, lights,
                                  shapes, default_map);
            shapes.push_back(s);
        } else if (name == "texture") {
            std::string id = child.attribute("id").value();
            if (texture_map.find(id) != texture_map.end()) { Error(std::string("Duplicated texture ID:") + id); }
            ...
            if (type == "envmap") {
                ...
                for (auto grand_child : child.children()) {
                    std::string name = grand_child.attribute("name").value();
                    if (name == "filename") {
                        filename = parse_string(grand_child.attribute("value").value(), default_map);
                    } else if (name == "toWorld" || name == "to_world") {
                ...
                if (filename.size() > 0) {
                    Texture<Spectrum> t =
                        make_image_spectrum_texture("__envmap_texture__", filename, texture_pool, 1, 1);
                ...
                Material m      = Lambertian{ make_constant_spectrum_texture(make_zero_spectrum()) };
    ...
    return std::make_unique<Scene>(embree_device, camera, materials, shapes, lights, media, envmap_light_id,
                                   texture_pool, options, filename);
}
