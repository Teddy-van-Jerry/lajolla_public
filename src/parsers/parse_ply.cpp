...
TriangleMesh parse_ply(const fs::path& filename, const Matrix4x4& to_world) {
    ...
    } catch (const std::exception& e) { Error(std::string("Vertex positions not found in ") + filename.string()); }
    try {
    ...
    } catch (const std::exception& e) { Error(std::string("Vertex indices not found in ") + filename.string()); }
    ...
    if (uvs) {
        ...
        if (uvs->t == tinyply::Type::FLOAT32) {
            ...
            for (size_t i = 0; i < uvs->count; i++) { mesh.uvs[i] = Vector2{ data[2 * i], data[2 * i + 1] }; }
    ...
    if (faces->t == tinyply::Type::INT8) {
        ...
        for (size_t i = 0; i < faces->count; i++) {
            mesh.indices[i] = Vector3i{ data[3 * i], data[3 * i + 1], data[3 * i + 2] };
        }
