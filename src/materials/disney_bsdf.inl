...
Spectrum eval_op::operator()(const DisneyBSDF &bsdf) const {
    ...
    return wDiffuse * f_diffuse
         + wSheen * f_sheen
         + wMetal * f_metal
         + wClearcoat * f_clearcoat
         + wGlass * f_glass;
}
...
std::optional<BSDFSampleRecord>
    ...
    if (Xi < wD) {
        ...
        diffBSDF.roughness  = bsdf.roughness;
        Real newX = Xi / wD;
        return sample_bsdf_op{dir_in, vertex, texture_pool,
                              Vector2(newX, rnd_param_uv.y), rnd_param_w, dir}(diffBSDF);
    }
    Xi -= wD;
    ...
    if (Xi < wM) {
        ...
        Real newX = Xi / wM;
        return sample_bsdf_op{dir_in, vertex, texture_pool,
                              Vector2(newX, rnd_param_uv.y), rnd_param_w, dir}(metalBSDF);
    }
    Xi -= wM;
    ...
    if (Xi < wC) {
        ...
        Real newX = Xi / wC;
        return sample_bsdf_op{dir_in, vertex, texture_pool,
                              Vector2(newX, rnd_param_uv.y), rnd_param_w, dir}(coatBSDF);
    }
    Xi -= wC;
    ...
    // Glass
    {
        ...
        Real newX = Xi / wG;
        return sample_bsdf_op{dir_in, vertex, texture_pool,
                              Vector2(newX, rnd_param_uv.y), rnd_param_w, dir}(glassBSDF);
    }
}
