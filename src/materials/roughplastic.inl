...
Spectrum eval_op::operator()(const RoughPlastic& bsdf) const {
    ...
    if (n_dot_out <= 0 || n_dot_h <= 0) { return make_zero_spectrum(); }

    Spectrum Kd    = eval(bsdf.diffuse_reflectance, vertex.uv, vertex.uv_screen_size, texture_pool);
...
Real pdf_sample_bsdf_op::operator()(const RoughPlastic& bsdf) const {
    ...
    Real n_dot_out      = dot(frame.n, dir_out);
    Real n_dot_h        = dot(frame.n, half_vector);
    if (n_dot_out <= 0 || n_dot_h <= 0) { return 0; }

    Spectrum S = eval(bsdf.specular_reflectance, vertex.uv, vertex.uv_screen_size, texture_pool);
    Spectrum R = eval(bsdf.diffuse_reflectance, vertex.uv, vertex.uv_screen_size, texture_pool);
...
std::optional<BSDFSampleRecord> sample_bsdf_op::operator()(const RoughPlastic& bsdf) const {
    ...
    if (rnd_param_w < spec_prob) {
        ...
        // Lambertian sampling
        return BSDFSampleRecord{ to_world(frame, sample_cos_hemisphere(rnd_param_uv)), Real(0) /* eta */,
                                 Real(1) /* roughness */ };
    }
}

TextureSpectrum get_texture_op::operator()(const RoughPlastic& bsdf) const { return bsdf.diffuse_reflectance; }
