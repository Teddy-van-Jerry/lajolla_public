...
TableDist2D make_table_dist_2d(const std::vector<Real>& f, int width, int height) {
    ...
    for (int y = 0; y < height; y++) {
        ...
        for (int x = 0; x < width; x++) {
            cdf_rows[y * (width + 1) + (x + 1)] = cdf_rows[y * (width + 1) + x] + f[y * width + x];
        }
        Real integral = cdf_rows[y * (width + 1) + width];
        if (integral > 0) {
            // Normalize
            for (int x = 0; x < width; x++) { cdf_rows[y * (width + 1) + x] /= integral; }
            ...
            // Setup the pmf/pdf
            for (int x = 0; x < width; x++) { pdf_rows[y * width + x] = f[y * width + x] / integral; }
        } else {
    ...
    if (total_values > 0) {
        // Normalize
        for (int y = 0; y < height; y++) { cdf_marginals[y] /= total_values; }
    ...
    for (int y = 0; y < height; y++) { cdf_rows[y * (width + 1) + width] = 1; }

    return TableDist2D{ cdf_rows, pdf_rows, cdf_marginals, pdf_marginals, total_values, width, height };
}
...
Vector2 sample(const TableDist2D& table, const Vector2& rnd_param) {
    ...
    const Real* y_ptr = std::upper_bound(table.cdf_marginals.data(), table.cdf_marginals.data() + h + 1, rnd_param[1]);
    ...
    if (cdf[x_offset + 1] - cdf[x_offset] > 0) { dx /= (cdf[x_offset + 1] - cdf[x_offset]); }
