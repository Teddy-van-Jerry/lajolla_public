...
/// wildly known as "GGX" (seems to stand for "Ground Glass X"
/// https://twitter.com/CasualEffects/status/783018211130441728).
///
/// We will use a generalized version of GGX called Generalized Trowbridge and Reitz (GTR),
/// proposed by Brent Burley and folks at Disney
/// (https://www.disneyanimation.com/publications/physically-based-shading-at-disney/) as our normal distribution
/// function. GTR2 is equivalent to GGX.
...
inline Real GGX(Real n_dot_h, Real roughness) { return GTR2(n_dot_h, roughness); }
...
inline Real GGX(const Vector3& h, Real roughness, Real anisotropic) {
    ...
    return Real(1.0) / (c_PI * alpha_x * alpha_y *
                        pow(h.x * h.x / alpha_x / alpha_x + h.y * h.y / alpha_y / alpha_y + h.z * h.z, Real(2.0)));
}
...
inline Vector3 sample_visible_normals(const Vector3& v_local, Real alpha_x, Real alpha_y, const Vector2& rnd_param) {
    // If the incoming dir is below z=0, flip it (the distribution is symmetrical).
    if (v_local.z < 0) { return -sample_visible_normals(-v_local, alpha_x, alpha_y, rnd_param); }

    Vector3 v_hemi = normalize(Vector3(alpha_x * v_local.x, alpha_y * v_local.y, v_local.z));
    ...
    Vector3 diskN(t1, t2, std::sqrt(std::max(Real(0), 1 - t1 * t1 - t2 * t2)));

    Frame hemiFrame(v_hemi);
    Vector3 n_hemi = to_world(hemiFrame, diskN);

    return normalize(Vector3(alpha_x * n_hemi.x, alpha_y * n_hemi.y, std::max(Real(0), n_hemi.z)));
}
